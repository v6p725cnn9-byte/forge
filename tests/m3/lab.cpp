...
bool ShadowWalkLab::create_shadow_map(rhi::Host& host, [[maybe_unused]] render::Renderer& renderer)
{
...
bool ShadowWalkLab::setup(rhi::Host& host, [[maybe_unused]] render::Renderer& renderer, Camera& camera)
{
                                     ...
                                     Uint32 width, Uint32 height, Camera& camera, bool)
{
    ...
    if (debug_.bloom > 0.001f && !renderer.apply_bloom(command, 1.0f)) return rhi::FrameResult::failed;
    if (!renderer.apply_tonemap(command, swapchain, debug_.exposure, debug_.bloom)) return rhi::FrameResult::failed;
    return rhi::FrameResult::presented;
}

void ShadowWalkLab::teardown(rhi::Host& host, [[maybe_unused]] render::Renderer& renderer)
{
