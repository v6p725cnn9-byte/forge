...
int main()
{
    ...
    check(vm.run_string("id = SpawnPlayer(1.5, 2.0, 3.5)\n"
                        ...
                        "boot"),
          vm.last_error().c_str());
    check(world.actors.alive_players() == 1 && world.actors.player(0) && world.actors.player(0)->name == "CJ",
          "SpawnPlayer");
    check(std::abs(world.actors.player(0)->position.x - 1.5f) < 1e-5f, "player x");
    check(world.actors.alive_vehicles() == 1 && std::abs(world.actors.vehicle(0)->yaw - 90.0f) < 1e-4f,
          "SpawnVehicle heading");
    check(world.actors.alive_markers() == 1 && world.actors.marker(0)->size > 1.0f, "CreateMarker");
    check(world.actors.alive_labels() == 1 && world.actors.labels()[0].text == "Hello", "Create3DTextLabel");
    check(vm.run_string("x,y,z = GetPlayerPos(0)\nassert(math.abs(x-1.5)<0.001)\nSetPlayerPos(0, 9, 1, 2)\n",
                        "move"),
          vm.last_error().c_str());
    check(std::abs(world.actors.player(0)->position.x - 9.0f) < 1e-5f, "SetPlayerPos");
    ...
    check(world.actors.alive_players() == 2, "on_init spawn");
    ...
    check(world.actors.alive_markers() == 0, "DestroyMarker");
