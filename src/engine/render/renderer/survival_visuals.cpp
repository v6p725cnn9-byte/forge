...
{
    assets::Scene scene;
    if (!assets::catalog().load_gltf(path, scene, error)) return false;
                          ...
                          int headless_player)
{
    ...
    for (const auto& ghost : snapshot.entities) {
        ...
        if (ghost.kind == net::Kind::Player) {
        ...
        } else if (ghost.kind >= net::Kind::Stick && ghost.kind <= net::Kind::Loot) {
            int prop = 5;
            glm::vec4 tint{1};
            switch (ghost.kind) {
            ...
            case net::Kind::Loot: prop = 9; tint = {.82f,.72f,.28f,1}; break;
            default: break;
            }
